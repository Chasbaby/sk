 ...
 * @date 2023-2
 */
public class OfflineRecommend implements Serializable {
    public final static Logger logger = LoggerFactory.getLogger(OfflineRecommend.class);

    public void offlineService(){
        SparkSession spark = buildSparkSession("local[2]", "OffLineRecommend", true);
        Dataset<?> sights_record_score = readFromMysql(spark, "sights_record_score");
        Dataset<Row> dataset = sights_record_score.toDF("userId", "sightsId", "score", "createTime");
        JavaRDD<SightsRecordScore> sightsRecordScore = dataset
                .toJavaRDD()
                .map(item -> new SightsRecordScore(item.getLong(0), item.getLong(1), item.getDouble(2)));

        JavaRDD<Tuple3<Long, Long, Double>> ratingRDD = sightsRecordScore
                .map((Function<SightsRecordScore, Tuple3<Long, Long, Double>>)
                        f -> new Tuple3<>(f.getUserId(), f.getSightsId(), f.getScore())).cache();
        ...
        // 核心计算过程
        // 1. 训练隐语义模型
        JavaRDD<Rating> dataTrain = ratingRDD.map(x -> new Rating(x._1().intValue(),x._2().intValue(), x._3().doubleValue()));
