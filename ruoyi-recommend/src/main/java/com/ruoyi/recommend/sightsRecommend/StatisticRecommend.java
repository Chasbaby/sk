...
public class StatisticRecommend {
    public static void  StatisticService(){
        ...
        Dataset<Row> rateMoreSightsIdDF = spark.sql("select sightsId ,count(sightsId) as count from ratings " +
                "group by sightsId order by count desc");
        ...
        Dataset<Row> ratingOfYearMonthDF = spark.sql("select sightsId,score, " +
                "changeData(createTime) as yearmonth from ratings");
        ratingOfYearMonthDF.createOrReplaceTempView("ratingOfMonth");
        Dataset<Row> rateMoreRecentlySights = spark.sql("select sightsId, count(sightsId) as count, " +
                "yearmonth from ratingOfMonth group by yearmonth, " +
                "sightsId order by yearmonth desc, count desc");
        writeToMysql(rateMoreRecentlySights,"RateMoreRecentlySights","overwrite");
        //3.优质景点统计
        Dataset<Row> averageSightsDF = spark.sql("select sightsId, avg(score) as avg from ratings group by sightsId order by avg desc");
